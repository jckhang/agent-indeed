...
| `/manager/tasks/{taskId}/candidates` | `GET /v1/tasks/{taskId}/candidates` | `taskId`, `limit`, optional `includeScoreBreakdown` | `agentId`, `eligible`, `rank`, `matchingTraceId`, `identityTier`, `matchedSkills`, `missingRequiredSkills`, `complianceStatus`, `eligibilityChecks`, optional `scoreBreakdown` | Ready on the active matching-contract branch; UI should handle `409 TASK_MATCH_NOT_READY` while snapshots are generating |
