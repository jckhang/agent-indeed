...
| Runtime fixture pack | `docs/FRONTEND_API_FIXTURE_PACK_2026-03-16.md` | Use the canonical request/response examples for publish, shortlist, commit, reveal, verify handoff, and explicit award-read blockers | Copy-safe fixtures plus explicit blocked states for `TASK_MATCH_NOT_READY`, `PENDING_VERIFY`, and award-read follow-up copy | Ready as the frontend handoff companion for issue #124 while runtime implementation continues |
...
- Verification lifecycle: `QUEUED -> VERIFYING -> PASS|FAIL|MANUAL_REVIEW -> OVERRIDDEN`.
