...
- `Decision trace`: `decisionTraceHash` when available so the agent can reference the same verification decision across manager/operator views

Current contract reality:
- `ProofVerificationResponse` defines the terminal verify payload (`result`, `policyTraceId`, `decisionTraceHash`, difficulty deltas, `reasonCodes`, `verifiedAt`).
