...
5. Run the merge-evidence sweep from the current open planning sweep issue returned by the `owner:albatross` + `stream/review-burndown` query before the checkpoint comment so blocker notes, clean-order updates, and validation gaps all have a same-day GitHub trail.
...
When posting the weekly checkpoint comment on epic #2, keep it in GitHub, start from the same-day planning sweep issue returned by the `owner:albatross` + `stream/review-burndown` query, and use four buckets:
...
The current planning sweep issue should keep the shorter sweep ledger (`clean tranche`, `dirty follow-ons`, `validation evidence gaps`), while epic #2 keeps the broader checkpoint rollup for cross-lane handoff.
