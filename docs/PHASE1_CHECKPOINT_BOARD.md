...
| --- | --- | --- | --- | --- | --- |
| M1 Contract Freeze + Upload | 2026-03-20 | albatross-dev-agent + kestrel | [Milestone issues](https://github.com/jckhang/agent-indeed/issues?q=is%3Aissue+milestone%3A%22M1+Contract+Freeze+%2B+Upload%22) | [Milestone PRs](https://github.com/jckhang/agent-indeed/pulls?q=is%3Apr+milestone%3A%22M1+Contract+Freeze+%2B+Upload%22) | Keep OpenSpec/API/contracts, milestone hygiene, and checkpoint rollups aligned to the merged local execution baseline (`docs/RUNTIME_EXECUTION_HANDOFF.md`) plus the live QA sweep issue query for contract/runtime drift evidence. |
| M2 Matching + Bidding Baseline | 2026-03-27 | kestrel + lanzhou-fe-agent | [Milestone issues](https://github.com/jckhang/agent-indeed/issues?q=is%3Aissue+milestone%3A%22M2+Matching+%2B+Bidding+Baseline%22) | [Milestone PRs](https://github.com/jckhang/agent-indeed/pulls?q=is%3Apr+milestone%3A%22M2+Matching+%2B+Bidding+Baseline%22) | Preserve the merged publish/match/commit/reveal/verify/award runtime plus the frontend consumer handoff as the default executable baseline (#110, #136, #145). |
| M3 Verify + Agent Flow | 2026-04-03 | kestrel + lanzhou-fe-agent + avery | [Milestone issues](https://github.com/jckhang/agent-indeed/issues?q=is%3Aissue+milestone%3A%22M3+Verify+%2B+Agent+Flow%22) | [Milestone PRs](https://github.com/jckhang/agent-indeed/pulls?q=is%3Apr+milestone%3A%22M3+Verify+%2B+Agent+Flow%22) | Convert the merged smoke path into repeatable happy/negative QA evidence and keep verifier/audit outputs tied to issue #11 handoff rules (`docs/QA_CONTRACT_DRIFT_SWEEP_2026-03-18.md`, issue #11). |
| M4 Audit + Beta Readiness | 2026-04-10 | albatross-dev-agent + kestrel + avery | [Milestone issues](https://github.com/jckhang/agent-indeed/issues?q=is%3Aissue+milestone%3A%22M4+Audit+%2B+Beta+Readiness%22) | [Milestone PRs](https://github.com/jckhang/agent-indeed/pulls?q=is%3Apr+milestone%3A%22M4+Audit+%2B+Beta+Readiness%22) | Final issue #11 evidence pack, audit trail sign-off, and closed-beta security readiness closure. |
...
- [Planning sweep issues for checkpoint rollups](https://github.com/jckhang/agent-indeed/issues?q=is%3Aissue+is%3Aopen+label%3A%22owner%3Aalbatross%22+label%3A%22stream%2Freview-burndown%22)
- [QA sweep issues for contract/runtime drift](https://github.com/jckhang/agent-indeed/issues?q=is%3Aissue+is%3Aopen+label%3A%22owner%3Aavery%22+label%3A%22stream%2Fruntime-handoff%22)
...
5. Start from the current open planning sweep issue returned by the `owner:albatross` + `stream/review-burndown` query so the checkpoint comment inherits the same-day clean tranche, dirty follow-on, and validation-gap notes without depending on a closed weekly issue id.
...
When posting the weekly checkpoint comment on epic #2, keep it in GitHub, start from the same-day planning sweep issue plus the paired QA sweep when contract/runtime drift changed, and use four buckets:
...
The current open planning sweep issue should keep the shorter sweep ledger (`clean tranche`, `dirty follow-ons`, `validation evidence gaps`), while epic #2 keeps the broader checkpoint rollup for cross-lane handoff. Pair it with the current QA sweep issue whenever contract/runtime drift or smoke evidence changed that week.
