...
| Issue #10 audit events and award trace | Award + audit ledger | Emit `POMW_VERIFIED` and `TASK_AWARDED`, include `audit_id`, `actor_type`, `decisionTraceHash`, and reputation writeback status | Beta reviewers cannot validate award correctness without a stitched audit trail |
