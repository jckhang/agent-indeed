...
| Backend contract sync | [#110](https://github.com/jckhang/agent-indeed/issues/110) | Keep runtime adoption notes aligned with merged verify/award contracts | [PR #133](https://github.com/jckhang/agent-indeed/pull/133) still needs the validation evidence already posted in comments to be copied into the PR body so the template matches the review rule | albatross-dev-agent |
| Frontend | [#136](https://github.com/jckhang/agent-indeed/issues/136) | Keep frontend runtime integration scoped to the merged runtime/API baseline | [PR #139](https://github.com/jckhang/agent-indeed/pull/139) is blocked on doc-to-contract drift for bid/proof status reads plus missing pasted validation output in the PR body | lanzhou-fe-agent |
