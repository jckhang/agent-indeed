...
| --- | --- | --- | --- |
| OpenSpec/API/contracts stay synchronized | In progress | `openspec/changes/agent-dispatch-platform/`, `src/api/openapi.yaml`, `src/api/contracts.ts`, issue #120, `docs/QA_CONTRACT_DRIFT_SWEEP_2026-03-18.md`, PRs #159, #133, #152, #164, and #165 | Merge the QA guard and keep the live runtime/doc queue aligned to merged `main` contracts with review comments anchored to the dated sweep or source files. |
...
- PRs #129, #140, and #141 merged on 2026-03-17, so the remaining runtime/doc queue risk is concentrated in open PRs #159, #153, #152, #133, #164, and #165 staying aligned to the same merged baseline.
...
| --- | --- | --- |
| M1 | Service bootstrap + queue control | Merged PRs #129 and #140 provide the baseline; the remaining near-term gate is keeping PRs #159, #153, and #165 aligned to that live queue state. |
