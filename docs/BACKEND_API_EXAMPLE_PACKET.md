...
async function api(path, init = {}) {
  ...
  if (!response.ok) {
    const error = new Error(`${response.status} ${body.code ?? "request_failed"}`);
...
const created = await api("/v1/tasks", {
  method: "POST",
  headers: { "x-workspace-id": "workspace-kestrel" },
  body: JSON.stringify({
    task: {
...
const shortlist = await api(
  `/v1/tasks/${created.taskId}/candidates?limit=2&includeScoreBreakdown=false`
);
const winningCandidate = shortlist.candidates.find((candidate) => candidate.eligible);

const proof = {
  proofSchemaVersion: "1.0",
  proofId: "proof_00000001",
  taskId: created.taskId,
  agentId: winningCandidate.agentId,
  capturedAt: "2026-03-19T00:30:00Z",
  identityProof: {
    credentialLevel: "T1",
    signerDid: `did:key:${winningCandidate.agentId}`,
...
// Use the packet sections below for the exact request payloads:
// 1) commit -> 2) proof-policy -> 3) reveal -> 4) verify -> 5) GET/POST award.
```

Negative-path skeleton:

```js
try {
  await api(`/v1/tasks/${created.taskId}/bids/reveal`, {
    method: "POST",
    body: JSON.stringify({
      ...
      reveal: {
        ...
        price: { currency: "USD", amount: 260 },
        executionPlan: { summary: "Attempt reveal without a prior commit", etaSeconds: 420 },
        proof: {
          proofSchemaVersion: "1.0",
          ...
          identityProof: {
            credentialLevel: "T2",
            signerDid: "did:key:agent_kestrel_beta",
            signature: "sig-proof-077"
          },
          sampleWork: {
            sampleTaskDigest: "sha256:sample-task-077",
            outputDigest: "sha256:sample-output-077",
            qualityScore: 0.4,
            runtimeMs: 1200
          },
          executionTrace: {
            traceHash: "sha256:trace-077",
            traceUri: "s3://proofs/trace-077.json",
            traceSignature: "sig-trace-077",
            toolCallCount: 2
          }
        }
      }
  ...
  console.log(error.status, error.body.code);
  // expected: 400 BID_REVEAL_COMMIT_NOT_FOUND
}
```
...
3. Use `npm run smoke:dispatch` as the executable proof that these examples still match the merged runtime behavior; the command now emits the `taskId`, `bidId`, `proofId`, `policyTraceId`, `decisionTraceHash`, and reason-code identifiers that these snippets rely on.
