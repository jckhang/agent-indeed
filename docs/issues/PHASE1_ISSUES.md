...
- P1-33 #101: https://github.com/jckhang/agent-indeed/issues/101
...
### P1-28 Run QA smoke pass on next merged tranche
...
Acceptance criteria:
- QA can execute the ready-now smoke checks against the latest merged manager and agent slices.
- Smoke fixtures and checkpoints reuse the shared contract vocabulary without inventing new request shapes.
- Any still-blocked assertion is linked back to the owning backend PR or issue.

### P1-33 Prewrite MVP E2E assertion matrix from merged docs
...
Acceptance criteria:
- The matrix lists happy-path checkpoints plus the minimum negative scenarios (`invalid signature`, `no commit`, `proof FAIL`, `manual review`, `award blocked`).
- Each blocked assertion names the contract PR or issue that must merge first.
- QA can start smoke and E2E implementation from one shared vocabulary without inventing new field or enum names.
>>>>>>> 0d84882 ([P1-33] Prewrite MVP E2E assertion matrix)
