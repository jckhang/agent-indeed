...
- 增补 merge-evidence sweep 回写规则，要求当前 `owner:albatross` + `stream/review-burndown` 查询返回的开放 planning sweep issue 承接同日队列巡检，epic #2 保留跨 lane checkpoint 汇总。
...
- 明确当前 planning sweep issue 与 epic #2 的 GitHub 评论分工，减少 review-burndown 期间重复回贴和状态漂移。
