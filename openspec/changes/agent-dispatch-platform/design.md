...
5. PoMW 策略按身份层级动态调节
   ...
   - `ProofPack` 至少由三段可独立校验的数据构成：identity proof、sample work、execution trace；anti-sybil challenge 作为可选增强段。
   - verifier 输出统一状态：`passed`、`failed`、`needs_review`，并附带稳定 `reason_code[]`，供 audit / UI / replay 直接消费。
...
- 校验请求必须绑定已持久化的 `policy_trace_id`，避免 verify 阶段重新计算策略导致 award/audit 回放不一致。
- 校验响应必须返回结构化 verdict（status、reason codes、difficulty delta、decision trace id），而不是只返回布尔结论。
